...
func AiRequestWithWord(word string) (string, error) {
	...
	response, err := callDeepSeekWithHTTP("解析单词" + word + "需要给出这个单词的常用意思、不同领域存在的意思、发音音标、其它形式怎么变换记忆、简单常用的案例句子方便记忆。按照markdown文本格式输出。注意文本不要有特殊字符。")
	if err == nil {
		slog.Info("进入模块 toAi", "function", "DoSomething")
	...
	slog.Info("进入模块 toAi", "function", "响应失败")
	return "", fmt.Errorf("响应失败: %w", err)
